...
public class StudentFeeForm extends JFrame implements ActionListener{
	...
	contentPane.add(t3);
        ...
        try{
            conn c = new conn();
            ResultSet rs = c.s.executeQuery("select * from student1 where roll_no= '"+c1.getSelectedItem()+"'");
            while(rs.next()){
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            ...
            if(ae.getSource() == b1){
                try{
                conn con = new conn();
                String sql = "insert into fee(roll_no, name, father_name, course, branch, semester, fee_paid) values(?, ?, ?, ?, ?, ?, ?)";
